...
% Set byte size
bytesize     = SDCpckg.Reg.utils.getByteSize(headerIn.precision);

SDCpckg.Reg.io.NativeBin.serial.FileAlloc(dirnameOut,headerIn);
header       = headerIn;
header.size  = [1 prod(headerIn.size)];
SDCpckg.Reg.io.NativeBin.serial.HeaderWrite(dirnameOut,header);
...
SDCpckg.Reg.io.NativeBin.serial.HeaderWrite(dirnameOut,headerIn);
end
