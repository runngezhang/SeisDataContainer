...
% Set byte size
header          = SDCpckg.Reg.io.NativeBin.serial.HeaderRead(dirnameIn);
bytesize        = SDCpckg.Reg.utils.getByteSize(header.precision);
headerx         = header;
headerx.complex = 1;
headerx.size    = [1 prod(header.size)];
SDCpckg.Reg.io.NativeBin.serial.FileAlloc(dirnameOut,headerx);
SDCpckg.Reg.io.NativeBin.serial.HeaderWrite(dirnameOut,headerx);
