...
if(isscalar(B))
    ...
    headerA = SDCpckg.Reg.io.NativeBin.serial.HeaderRead(A);
    if(headerA.complex)
        error('Epic fail: the first input is complex')
    end        
    % Set byte size
    bytesize          = SDCpckg.Reg.utils.getByteSize(headerA.precision);
...
elseif(isdir(B))
    ...
    SDCpckg.Reg.io.NativeBin.serial.FileCopy(B,dirnameOut)
    movefile([dirnameOut filesep 'real'],[dirnameOut filesep 'imag'])        
else
...
if(isscalar(A))
    ...
    headerB = SDCpckg.Reg.io.NativeBin.serial.HeaderRead(B);
    % Set byte size
    bytesize       = SDCpckg.Reg.utils.getByteSize(headerB.precision);
...
elseif(isdir(A))
    ...
    SDCpckg.Reg.io.NativeBin.serial.FileCopy(A,dirnameOut)
    headerOut = headerA;
    headerOut.complex = 1;
    SDCpckg.Reg.io.NativeBin.serial.HeaderWrite(dirnameOut,headerOut);
else
