...
if distribute
    SDCpckg.Reg.io.NativeBin.serial.DataWrite(dirname,filename,lx,file_precision);
else
    assert(isvector(localindx),'localindx must be a vector')
    SDCpckg.Reg.io.acquireIOlock(dirname);
    SDCpckg.Reg.io.NativeBin.serial.DataWriteLeftChunk(dirname,filename,lx,...
        size(x),localindx,[],file_precision);
    SDCpckg.Reg.io.releaseIOlock(dirname);
end
