...
if ~hdrin.distributedIO % serial redirect to serial transpose
    ...
    if nargin>3
        ...
        partition = SDCpckg.Reg.utils.defaultDistribution(sizeIn(end));
    end
    ...
    hdrout = SDCpckg.Reg.transposeHeaderStruct(hdrin,hdrin.distribution.dim);
    hdrout = SDCpckg.Reg.addDistHeaderStruct(hdrout,hdrin.dims-hdrin.distribution.dim,partition);
    hdrout = SDCpckg.Reg.addDistFileHeaderStruct(hdrout,distdirs);
    hdcmplx = hdrin.complex;
    fprecision = hdrin.precision;

    % Allocate file
    SDCpckg.Reg.io.NativeBin.dist.FileAlloc(dirnameOut,hdrout);
    ...
    % composites
    distdirin = SDCpckg.Reg.utils.Cell2Composite(hdrin.directories);
    distdirout = SDCpckg.Reg.utils.Cell2Composite(hdrout.directories);
        ...
        for s=1:pds
            ...
            if labindex==sender
                for l=1:numlabs
                    ...
                    if count>0
                        bfr = SDCpckg.Reg.io.NativeBin.serial.DataReadStriped(distdirin,'real',...
                            [dds(sender) pde],start,count,skip,fprecision,fprecision);
                        if hdcmplx
                            dummy = SDCpckg.Reg.io.NativeBin.serial.DataReadStriped(distdirin,'imag',...
                                [dds(sender) pde],start,count,skip,fprecision,fprecision);
            ...
            % write slice
            SDCpckg.Reg.io.NativeBin.dist.DataWriteLeftSlice(1,distdirout,'real',real(LB),...
                hdrout.size,[dde(labindex) pds],[],1,s,fprecision);
            if hdcmplx
                SDCpckg.Reg.io.NativeBin.dist.DataWriteLeftSlice(1,distdirout,'imag',imag(LB),...
                    hdrout.size,[dde(labindex) pds],[],1,s,fprecision);
            end
        end
    end
