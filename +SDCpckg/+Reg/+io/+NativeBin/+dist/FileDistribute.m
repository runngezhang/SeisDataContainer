...
% Update headers
partition = SDCpckg.Reg.utils.defaultDistribution(hdrin.size(distdim));
hdrout = SDCpckg.Reg.addDistHeaderStruct(hdrin,distdim,partition);
hdrout = SDCpckg.Reg.addDistFileHeaderStruct(hdrout,dirsout);
sldims = hdrin.size(distdim+1:end);

% Allocate file
SDCpckg.Reg.io.NativeBin.dist.FileAlloc(dirout,hdrout);

% Copy file
dirnames = SDCpckg.Reg.utils.Cell2Composite(hdrout.directories);
csize_out = SDCpckg.Reg.utils.Cell2Composite(hdrout.distribution.size);
cindx_rng_out = SDCpckg.Reg.utils.Cell2Composite(hdrout.distribution.indx_rng);
spmd
for s=1:prod(sldims)
    slice = SDCpckg.Reg.utils.getSliceIndexS2V(sldims,s);
    x=SDCpckg.Reg.io.NativeBin.dist.DataReadLeftSlice(0,dirin,'real',...
        hdrout.size,[],cindx_rng_out,hdrout.distribution.dim,hdrout.distribution.partition,...
        slice,hdrout.precision,hdrout.precision);
    SDCpckg.Reg.io.NativeBin.dist.DataWriteLeftSlice(1,dirnames,'real',x,...
        hdrout.size,csize_out,[],hdrout.distribution.dim,...
        slice,hdrout.precision);
    if hdrout.complex
        x=SDCpckg.Reg.io.NativeBin.dist.DataReadLeftSlice(0,dirin,'imag',...
            hdrout.size,[],cindx_rng_out,hdrout.distribution.dim,hdrout.distribution.partition,...
            slice,hdrout.precision,hdrout.precision);
        SDCpckg.Reg.io.NativeBin.dist.DataWriteLeftSlice(1,dirnames,'imag',x,...
            hdrout.size,csize_out,[],hdrout.distribution.dim,...
            slice,hdrout.precision);
    end
end
