...
if header.distributedIO
    ...
    spmd
        ...
        if header.complex
            dummy=SDCpckg.Reg.io.NativeBin.dist.DataReadLeftSlice(1,dirnames,'imag',...
                header.size,csize,[],header.distribution.dim,header.distribution.partition,...
                slice,header.precision,x_precision);
            x=complex(x,dummy);
        end
    ...
    spmd
        x=SDCpckg.Reg.io.NativeBin.dist.DataReadLeftSlice(0,dirname,'real',...
            header.size,[],cindx_rng,header.distribution.dim,header.distribution.partition,...
            slice,header.precision,x_precision);
        if header.complex
            dummy=SDCpckg.Reg.io.NativeBin.dist.DataReadLeftSlice(0,dirname,'imag',...
                header.size,[],cindx_rng,header.distribution.dim,header.distribution.partition,...
                slice,header.precision,x_precision);
            x=complex(x,dummy);
        end
