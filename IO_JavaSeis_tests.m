function out = datacontests(varargin)
...
if parpool_size() == 0
    error('Parallel pool pool is not on!');
end
...
if nargin < 1
    suite = TestSuite.fromName(fullfile(SDCpckg.path,'tests','IO_JavaSeis_tests'));
else
...
for k = 1:numel(varargin)
    ...
    if ~isempty(name) && (name(1) == '-')
        warning('runtests:unrecognizedOption', 'Unrecognized option: %s', 'IO_JavaSeis_tests');
    else
